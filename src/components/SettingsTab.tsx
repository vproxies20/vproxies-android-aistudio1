...
export const SettingsTab: React.FC<SettingsTabProps> = ({
  ...
  const dnsOptionsList: Array<{ key: DnsOption; label: string; ip: string }> = [
    ...
    { key: 'PROXY', label: 'DNS via VProxies', ip: 'Auto' },
    { key: 'CUSTOM', label: 'Custom', ip: customDnsIp },
  ];
  ...
  return (
    <div className="space-y-4 pb-20 animate-in fade-in duration-300" id="settings_tab">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-100">Settings & Configuration</h2>
          <p className="text-xs text-slate-400">Manage VProxies credentials, routing tunnels, and secure DNS</p>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3 pb-3 border-b border-[#1E2D44]">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-slate-400">
              {accountInfo
                ? `Dedicated synchronization for ${accountInfo.identity}`
                : 'Sign in to automatically load your dedicated VProxies nodes'}
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-[#0F172A] border border-[#1E2D44]">
              <div>
                ...
                <div className="text-[11px] text-slate-400">
                  {accountInfo.packageName} · {accountInfo.remainingDays} days remaining
                </div>
              </div>
              <button
                onClick={onLogout}
            ...
            <div className="p-3 rounded-xl bg-[#0F172A] border border-emerald-500/30 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                  ...
                  Zero-Residual & Auto-Wipe on Logout
                </span>
              ...
              <p className="text-[11px] text-slate-300 leading-relaxed">
                Synced <strong>{proxiesCount} proxies</strong> from VProxies network. When you click <strong>"Sign Out"</strong>, all proxies and session cache will be 100% wiped from this device.
              </p>
              {onPurgeAllProxies && proxiesCount > 0 && (
                <div className="pt-1 flex items-center justify-between border-t border-[#1E2D44]">
                  <span className="text-[10px] text-slate-400">Clean local cache now?</span>
                  <button
          ...
          <div className="mt-3 space-y-3">
            <div>
              <label className="block text-[11px] font-semibold text-slate-400 mb-1">
                VProxies Username or Email
              </label>
              <input
                type="text"
                placeholder="saobien or user@vproxies.app"
                value={identityInput}
            ...
            <div className="p-3 rounded-xl bg-[#0F172A] border border-[#00E5FF]/30 space-y-1.5">
              <div className="flex items-center gap-1.5 text-xs font-bold text-[#00E5FF]">
                <ShieldCheck className="w-4 h-4" />
                <span>Zero-Residual & Auto-Wipe Policy</span>
              </div>
              <p className="text-[11px] text-slate-300 leading-relaxed">
                This app <strong>never permanently stores proxies locally</strong> without authorization. Signing in synchronizes your dedicated proxies, and signing out will <strong>permanently purge all proxy records</strong> from your device.
              </p>
            </div>
          </div>
        )}
      ...
      >
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-100">Network Routing Mode</h3>
            <p className="text-[11px] text-slate-400">Choose how network traffic is routed through the proxy tunnel</p>
          </div>
        </div>
          ...
          {[
            {
              ...
              desc: 'All applications and internet traffic route through Proxy',
            },
            {
              id: 1 as RoutingMode,
              title: 'Bypass LAN & Direct Local',
              desc: 'Exclude local private network addresses (192.168.x.x, 10.x.x.x)',
            },
            {
              id: 2 as RoutingMode,
              title: 'Selected Apps Only (Split Tunneling)',
              desc: 'Only designated applications will route through the tunnel',
            },
          ].map((mode) => (
            <label
              key={mode.id}
          ...
          <div className="mt-3 p-3 rounded-xl bg-[#0F172A] border border-[#1E2D44] flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-slate-300">
                Selected: <strong>{selectedApps.length} apps</strong>
              </span>
            </div>
            <button
      ...
      <div className="rounded-2xl bg-[#162032] border border-[#00E5FF]/25 p-4 shadow-lg space-y-3">
        <div className="flex items-center justify-between">
          ...
          >
            ...
            />
            <span>{updateCheckState.status === 'checking' ? 'Checking...' : 'Check for Updates'}</span>
          </button>
        </div>
            ...
            <div className="flex items-center justify-between">
              <span className="font-bold text-slate-200">
                {updateCheckState.hasNewerVersion
                  ? `Update available: ${updateCheckState.latestVersion}`
